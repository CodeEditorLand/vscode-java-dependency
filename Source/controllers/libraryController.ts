...
export class LibraryController implements Disposable {
	...
	public constructor(public readonly context: ExtensionContext) {
		this.disposable = Disposable.from(
			instrumentOperationAsVsCodeCommand(
				Commands.JAVA_PROJECT_ADD_LIBRARIES,
				() => this.addLibraries(),
			),
			instrumentOperationAsVsCodeCommand(
				Commands.JAVA_PROJECT_ADD_LIBRARY_FOLDERS,
				() => this.addLibraries(true),
			),
			instrumentOperationAsVsCodeCommand(
				Commands.JAVA_PROJECT_REMOVE_LIBRARY,
				(node: DataNode) =>
					node.uri && this.removeLibrary(Uri.parse(node.uri).fsPath),
			),
			instrumentOperationAsVsCodeCommand(
				Commands.JAVA_PROJECT_REFRESH_LIBRARIES,
	...
	public async addLibraries(canSelectFolders?: boolean) {
		...
		const results: Uri[] | undefined = await window.showOpenDialog({
			...
			canSelectMany: true,
			openLabel: canSelectFolders
				? "Select Library Folders"
				: "Select Jar Libraries",
			filters: canSelectFolders
				? { Folders: ["*"] }
				: { "Jar Files": ["jar"] },
		});
	...
	public async removeLibrary(removalFsPath: string) {
		...
		const removedPaths = _.remove(setting.include, (include) => {
			if (path.isAbsolute(include)) {
				...
				return (
					include === workspace.asRelativePath(removalFsPath, false)
				);
			}
		});
 ...
 */
function dedupAlreadyCoveredPattern(
	origin: string[],
	...update: string[]
): string[] {
