...
export class JavaType {
	public static readonly CLASS: JavaType = new JavaType(
		"Class",
		"class",
		"$(symbol-class)",
	);
	...
	public static fromDisplayName(label: string): JavaType | undefined {
		if (label?.startsWith("$")) {
			return JavaType.ALL.find(
				(javaType) => `${javaType.icon} ${javaType.label}` === label,
			);
		}
	...
	public static getDisplayNames(
		...
		return JavaType.ALL.filter(
			(javaType) => includeRecord || javaType !== JavaType.RECORD,
		).map((javaType) => {
			if (includeIcon) {
...
export async function newResource(node: DataNode): Promise<void> {
	...
	if (
		node.nodeData.kind === NodeKind.Project ||
		(node.nodeData.kind === NodeKind.PackageRoot &&
			!resourceRoots.includes(node.nodeData.name)) ||
	...
	const type = await window.showQuickPick(availableTypes, {
...
export async function newJavaFileWithSpecificType(
	javaType: JavaType,
	node?: DataNode | Uri,
): Promise<void> {
	sendInfo("", {
		"triggernewfilefrom":
			node instanceof Uri && node?.fsPath
				? "fileExplorer"
				: "javaProjectExplorer",
...
async function newJavaFile0(
	packageFsPath: string,
	javaType: JavaType | undefined,
) {
	if (!javaType) {
		return;
	}
...
async function newJavaFileWithContents(
	fsPath: string,
	javaType: JavaType,
	packageName: string,
) {
...
function isLanguageServerReady(): boolean {
	...
	if (
		extensionApi.serverMode !== "Standard" ||
		extensionApi.status !== "Started"
	) {
		return false;
	}
...
async function inferPackageFsPath(): Promise<string> {
	...
	)?.data?.map((sourcePath) => sourcePath.path);
...
async function isVersionLessThan(
	fileUri: string,
		...
		if (packageRoots.length < 1) {
			...
			if (!packageNode && node.uri) {
				...
				return getPackageRootPath(
					Uri.parse(packageNode.uri).fsPath,
					packageNode.name,
				);
			}
			return "";
...
export async function newPackage(
	...
	const { defaultValue: tempDefaultValue, packageRootPath } =
		(isUri
	...
	const packageName: string | undefined = await window.showInputBox({
		...
		validateInput: async (value: string): Promise<string> => {
			...
			if (
				await fse.pathExists(getNewPackagePath(packageRootPath, value))
			) {
...
async function getPackageInformationFromUri(
	uri: Uri,
...
async function getPackageInformationFromNode(
	node: DataNode,
): Promise<Record<string, string> | undefined> {
	...
	if (nodeKind === NodeKind.Project) {
		...
		return {
			packageRootPath,
			defaultValue: path
				.relative(packageRootPath, packagePath)
				.replace(/[/\\]/g, "."),
		};
	}
...
async function validateNewFileFolder(
	basePath: string,
	relativePath: string,
): Promise<string> {
...
async function getSourceRoots(): Promise<IListCommandResult | undefined> {
	try {
		const result = await commands.executeCommand<IListCommandResult>(
			Commands.EXECUTE_WORKSPACE_COMMAND,
