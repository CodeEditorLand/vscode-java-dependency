...
import {
	BuildArtifactTaskProvider,
	DeprecatedExportJarTaskProvider,
...
import {
	...
	updateExportTaskType,
} from "./tasks/buildArtifact/migration/utils";
...
export async function activate(context: ExtensionContext): Promise<void> {
	...
	workspace
		.findFiles("{*.gradle,*.gradle.kts,pom.xml,.classpath}", undefined, 1)
		.then((uris: Uri[]) => {
			if (uris && uris.length) {
				contextManager.setContextValue(
					Context.WORKSPACE_CONTAINS_BUILD_FILES,
					true,
				);
			}
		});
