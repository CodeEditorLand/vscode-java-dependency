...
import { CancellationToken, commands, Uri, workspace } from "vscode";
...
export namespace Jdtls {
	...
	export async function refreshLibraries(
		...
		return commands.executeCommand(
			Commands.EXECUTE_WORKSPACE_COMMAND,
			Commands.JAVA_PROJECT_REFRESH_LIB_SERVER,
			params,
		);
	}

	export async function getPackageData(
		params: IPackageDataParam,
	): Promise<INodeData[]> {
		...
		if (Settings.nonJavaResourcesFiltered()) {
			nodeData = nodeData.filter((data: INodeData) => {
				return (
					data.kind !== NodeKind.Folder && data.kind !== NodeKind.File
				);
			});
		}
	...
	export async function getMainClasses(
		params: string,
	): Promise<IMainClassInfo[]> {
		return (
			(await commands.executeCommand(
				Commands.EXECUTE_WORKSPACE_COMMAND,
				Commands.JAVA_PROJECT_GETMAINCLASSES,
				params,
			)) || []
		);
	}
	...
	export async function checkImportStatus(): Promise<boolean> {
		return (
			commands.executeCommand(
				Commands.EXECUTE_WORKSPACE_COMMAND,
				Commands.JAVA_PROJECT_CHECK_IMPORT_STATUS,
			) || false
		);
	}
	...
	export function resolveBuildFiles(): Promise<string[]> {
		return <Promise<string[]>>(
			executeJavaLanguageServerCommand(Commands.JAVA_RESOLVE_BUILD_FILES)
		);
	}
}
