...
export class Settings {
	public static initialize(context: ExtensionContext): void {
				...
				(e: ConfigurationChangeEvent) => {
					if (
						(e.affectsConfiguration(
							...
							Settings.syncWithFolderExplorer()) ||
						...
						e.affectsConfiguration("java.dependency.autoRefresh")
					) {
						syncHandler.updateFileWatcher(Settings.autoRefresh());
					} else if (
						e.affectsConfiguration("java.dependency.refreshDelay")
					) {
						// TODO: getInstance() should not have parameter if it means to be a singleton.
						DependencyExplorer.getInstance(
							contextManager.context,
						).dataProvider.setRefreshDebounceFunc(
		...
		context.subscriptions.push(
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_PACKAGE_LINKWITHFOLDER,
				Settings.linkWithFolderCommand,
			),
		);

		context.subscriptions.push(
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_PACKAGE_UNLINKWITHFOLDER,
				Settings.unlinkWithFolderCommand,
			),
		);

		context.subscriptions.push(
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_PACKAGE_CHANGETOFLATPACKAGEVIEW,
		...
		context.subscriptions.push(
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_PACKAGE_CHANGETOHIERARCHICALPACKAGEVIEW,
	...
	public static linkWithFolderCommand(): void {
		workspace
			.getConfiguration("java.dependency")
			.update("syncWithFolderExplorer", true, false);
	}

	public static unlinkWithFolderCommand(): void {
		workspace
			.getConfiguration("java.dependency")
			.update("syncWithFolderExplorer", false, false);
	}

	public static changeToFlatPackageView(): void {
		workspace
			.getConfiguration("java.dependency")
			.update("packagePresentation", PackagePresentation.Flat, false);
	}
	...
	public static switchNonJavaResourceFilter(enabled: boolean): void {
		workspace
			.getConfiguration("java.project.explorer")
			.update(
	...
	public static updateReferencedLibraries(
		libraries: IReferencedLibraries,
	): void {
		let updateSetting: string[] | Partial<IReferencedLibraries> = {
			include: libraries.include,
			exclude:
				libraries.exclude.length > 0 ? libraries.exclude : undefined,
			sources:
				Object.keys(libraries.sources).length > 0
					? libraries.sources
					: undefined,
		};
	...
	public static syncWithFolderExplorer(): boolean {
		return workspace
			.getConfiguration("java.dependency")
			.get("syncWithFolderExplorer", true);
	}
	...
	public static isHierarchicalView(): boolean {
		return (
			workspace
				.getConfiguration("java.dependency")
				.get("packagePresentation") === PackagePresentation.Hierarchical
		);
	}
	...
	public static nonJavaResourcesFiltered(): boolean {
		return !workspace
			.getConfiguration("java.project.explorer")
			.get<boolean>("showNonJavaResources", true);
	}
}
