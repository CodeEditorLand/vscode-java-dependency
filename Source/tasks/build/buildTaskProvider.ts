...
class BuildTaskTerminal implements Pseudoterminal {
	...
	constructor(
		private readonly definition: IBuildTaskDefinition,
	...
	async buildProjects(): Promise<number> {
		...
		let [includedPaths, excludedPaths, invalidPaths] = categorizePaths(
		...
		[includedPaths, invalidPaths] = getFinalPaths(
			includedPaths,
			excludedPaths,
			projectPaths,
		);
		...
		if (invalidPaths.length) {
			this.printList(
				"Following paths are skipped due to not matching any project root path:",
				invalidPaths,
			);
		}
...
export function categorizePaths(
	...
	for (const p of paths) {
		...
		if (
			actualPath === BuildTaskProvider.workspace ||
...
export function getFinalPaths(
	...
	includes = includes.filter((p) => {
		return !excludes.some(
			(excludePath) => path.relative(excludePath, p) === "",
		);
	});
