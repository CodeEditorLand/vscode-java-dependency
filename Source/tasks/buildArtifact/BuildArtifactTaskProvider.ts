...
const activeTerminalMap: Map<string, ExportJarTaskTerminal> = new Map<
	string,
	ExportJarTaskTerminal
>();
...
export async function executeExportJarTask(node?: INodeData): Promise<void> {
	...
	try {
		...
		if (!resolveJavaProjectExecutor) {
			throw new Error(
				ExportJarMessages.stepErrorMessage(
					ExportJarMessages.StepAction.FINDEXECUTOR,
		...
		tasks.executeTask(
			BuildArtifactTaskProvider.getDefaultTask(stepMetadata),
		);
	} catch (err) {
		if (err) {
			failMessage(`${err}`);
...
export class BuildArtifactTaskProvider implements TaskProvider {
		...
		if (!stepMetadata.workspaceFolder) {
			throw new Error(
				ExportJarMessages.fieldUndefinedMessage(
					ExportJarMessages.Field.WORKSPACEFOLDER,
					ExportJarStep.ResolveTask,
				),
			);
		}
	...
	public static async resolveExportTask(
		task: Task,
		type: string,
	): Promise<Task> {
		...
		try {
			...
			const resolvedTask: Task = new Task(
				...
				new CustomExecution(
					async (
						resolvedDefinition: IExportJarTaskDefinition,
					): Promise<Pseudoterminal> => {
						const stepMetadata: IStepMetadata = {
							entry: undefined,
		...
		for (const folder of folders) {
			try {
				...
				if (_.isEmpty(projectList)) {
					...
					elementList.push(
						"${" + ExportJarConstants.COMPILE_OUTPUT + "}",
					...
					new CustomExecution(
						async (
							...
							const stepMetadata: IStepMetadata = {
								entry: undefined,
								taskLabel:
									resolvedDefinition.label || folder.name,
								workspaceFolder: folder,
								projectList,
								steps: [],
								elements: [],
...
class ExportJarTaskTerminal implements Pseudoterminal {
	...
	public async open(
		...
		try {
			...
			if (!_.isEmpty(this.stepMetadata.elements)) {
				const outputFolderMap: Map<string, string[]> = new Map<
					string,
					string[]
				>();
				...
				this.stepMetadata.classpaths = await this.resolveClasspaths(
					outputFolderMap,
					artifactMap,
					testOutputFolderMap,
					testArtifactMap,
		...
		while (step !== ExportJarStep.Finish) {
			...
			if (!(await executor.execute(stepMetadata))) {
				...
				if (!previousStep) {
					throw new Error(
						ExportJarMessages.stepErrorMessage(
							ExportJarMessages.StepAction.GOBACK,
							step,
						),
					);
				}
				...
				switch (step) {
					...
					default:
						throw new Error(
							ExportJarMessages.stepErrorMessage(
								ExportJarMessages.StepAction.GOAHEAD,
								step,
							),
						);
				}
	...
	private async resolveClasspaths(
		...
		for (const glob of await globby(globPatterns)) {
			const tireNode: TrieNode<IUriData | undefined> | undefined =
				trie.find(
					Uri.file(platform() === "win32" ? toWinPath(glob) : glob)
						.fsPath,
					/* returnEarly = */ true,
				);
	...
	private getJarElementsFromClasspathMapping(
		matchResult: RegExpMatchArray,
		rawClasspathEntries: Map<string, string[]>,
		...
		if (projectName !== undefined) {
			...
			for (const classpath of entries) {
				result.push(
					this.toAbsolutePosixPath(
						matchResult.input.replace(matchResult[0], classpath),
					),
				);
			}
		} else {
