...
export class GenerateJarExecutor implements IExportJarStepExecutor {
	...
	private async generateJar(stepMetadata: IStepMetadata): Promise<boolean> {
		...
		if (stepMetadata.outputPath === "") {
			const outputUri: Uri | undefined = await window.showSaveDialog({
				defaultUri: Uri.file(
					join(folder.uri.fsPath, `${folder.name}.jar`),
				),
				filters: {
					"Java Archive": ["jar"],
				},
			...
			if (!outputPath) {
				throw new Error(
					ExportJarMessages.fieldUndefinedMessage(
						ExportJarMessages.Field.OUTPUTPATH,
						this.currentStep,
					),
				);
			}
			// Both the absolute path and the relative path (to workspace folder) are supported.
			destPath = isAbsolute(outputPath)
				? outputPath
				: join(folder.uri.fsPath, outputPath);
				...
				return new Promise<boolean>(async (resolve, reject) => {
					...
					if (mainClass === undefined) {
						return reject(
							new Error(
								ExportJarMessages.fieldUndefinedMessage(
									ExportJarMessages.Field.MAINCLASS,
									this.currentStep,
								),
							),
						);
	...
	private async generateClasspaths(
		...
		const dependencyItems: IJarQuickPickItem[] = await window.withProgress(
			...
			(_progress, token) => {
				return new Promise<IJarQuickPickItem[]>(
	...
	private async setStepMetadataFromOutputFolder(
		folderPath: string,
