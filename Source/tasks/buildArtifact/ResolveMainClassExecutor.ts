...
export class ResolveMainClassExecutor implements IExportJarStepExecutor {
	...
	private readonly currentStep: ExportJarStep =
		ExportJarStep.ResolveMainClass;
	...
	private async resolveMainClass(
		...
		const mainClasses: IMainClassInfo[] | undefined =
			await window.withProgress(
				...
				(_progress, token) => {
					return new Promise<IMainClassInfo[]>(
		...
		try {
			result = await new Promise<boolean>(async (resolve, reject) => {
				const pickBox = createPickBox<QuickPickItem>(
					"Export Jar : Determine main class",
