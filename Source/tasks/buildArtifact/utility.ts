...
export const stepMap: Map<ExportJarStep, IExportJarStepExecutor> = new Map<
	ExportJarStep,
	IExportJarStepExecutor
>([
...
export namespace ExportJarMessages {
	...
	export function fieldUndefinedMessage(
		field: Field,
		currentStep: ExportJarStep,
	): string {
	...
	export function stepErrorMessage(
		action: StepAction,
		currentStep: ExportJarStep,
	): string {
...
export function resetStepMetadata(
	resetTo: ExportJarStep,
...
export function createPickBox<T extends QuickPickItem>(
	title: string,
	placeholder: string,
	items: T[],
...
export function failMessage(message: string, option?: IMessageOption): void {
	...
	if (option === undefined) {
		...
		window
			.showErrorMessage(message, option.title, "Done")
			.then((result) => {
