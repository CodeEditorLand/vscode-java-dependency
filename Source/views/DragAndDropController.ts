...
export class DragAndDropController
	implements TreeDragAndDropController<ExplorerNode>
{
	 ...
	 */
	private addDragToEditorDataTransfer(
		node: ExplorerNode,
		treeDataTransfer: DataTransfer,
	) {
		if (
			(node instanceof PrimaryTypeNode || node instanceof FileNode) &&
			node.uri
		) {
			treeDataTransfer.set(
				Explorer.Mime.TextUriList,
				new DataTransferItem(node.uri),
			);
	 ...
	 * @param treeDataTransfer A map containing a mapping of the mime type of the corresponding transferred data.
	 */
	private addInternalDragDataTransfer(
		node: ExplorerNode,
		treeDataTransfer: DataTransfer,
	): void {
	 ...
	 */
	private async dropFromJavaProjectExplorer(
		target: ExplorerNode | undefined,
		uri: string,
	): Promise<void> {
		...
		if (target instanceof ContainerNode) {
			if (
				target.getContainerType() !== ContainerType.ReferencedLibrary ||
			...
			target instanceof PackageRootNode ||
			target instanceof PackageNode ||
	 ...
	 */
	private async dropFromFileExplorer(
		target: ExplorerNode | undefined,
		uris: string[],
	): Promise<void> {
		if (!this.isDroppableNode(target)) {
	...
	private isDraggableNode(node: DataNode | undefined): boolean {
		...
		if (
			...
			node instanceof PackageRootNode ||
		...
		while (parent) {
			if (parent instanceof ProjectNode) {
				...
				if (
					...
					ContainerType.ReferencedLibrary
				) {
					return (
