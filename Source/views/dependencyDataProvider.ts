...
} from "../tasks/buildArtifact/BuildArtifactTaskProvider";
...
export class DependencyDataProvider implements TreeDataProvider<ExplorerNode> {
	private _onDidChangeTreeData: EventEmitter<
		ExplorerNode | null | undefined
	> = new EventEmitter<ExplorerNode | null | undefined>();

	// tslint:disable-next-line:member-ordering
	public onDidChangeTreeData: Event<ExplorerNode | null | undefined> =
		...
		context.subscriptions.push(
			instrumentOperationAsVsCodeCommand(
				...
				async (node: INodeData) => {
					...
					if (uris.length >= 1) {
						commands.executeCommand(
							Commands.JAVA_PROJECT_CONFIGURATION_UPDATE,
							uris[0],
						);
					}
				},
	...
	): Promise<ExplorerNode[] | undefined | null> {
		if (!(await languageServerApiManager.ready())) {
			return [];
		}

		const children =
			!this._rootItems || !element
	...
	public async revealPaths(
		...
		const project = projects
			? <DataNode>(
					projects.find(
						(node: DataNode) =>
							node.path === projectNodeData?.path &&
							node.nodeData.name === projectNodeData?.name,
					)
				)
			: undefined;
