...
import {
	...
	newFile,
	newFolder,
	newJavaFileWithSpecificType,
	newPackage,
	newResource,
} from "../explorerCommands/new";
...
export class DependencyExplorer implements Disposable {
	...
	constructor(public readonly context: ExtensionContext) {
		...
		context.subscriptions.push(
			window.onDidChangeActiveTextEditor(
				(textEditor: TextEditor | undefined) => {
					if (
						this._dependencyViewer.visible &&
						textEditor?.document
					) {
			...
			this._dependencyViewer.onDidChangeVisibility(
			...
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_PACKAGE_REVEAL_IN_PROJECT_EXPLORER,
				async (uri: Uri) => {
					...
					this.reveal(
						uri,
						false /*force to reveal even the sync setting is turned off*/,
					);
				},
			),
			instrumentOperationAsVsCodeCommand(
				Commands.JAVA_PROJECT_EXPLORER_SHOW_NONJAVA_RESOURCES,
				async () => {
					Settings.switchNonJavaResourceFilter(true);
				},
			),
			instrumentOperationAsVsCodeCommand(
				Commands.JAVA_PROJECT_EXPLORER_HIDE_NONJAVA_RESOURCES,
				async () => {
		...
		context.subscriptions.push(
			this._dependencyViewer.onDidChangeSelection(
				(_e: TreeViewSelectionChangeEvent<ExplorerNode>) => {
					EventCounter.increase("didChangeSelection");
				},
			),
			this._dependencyViewer.onDidCollapseElement(
				(_e: TreeViewExpansionEvent<ExplorerNode>) => {
					EventCounter.increase("didCollapseElement");
				},
			),
			this._dependencyViewer.onDidExpandElement(
				(_e: TreeViewExpansionEvent<ExplorerNode>) => {
		...
		context.subscriptions.push(
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_PACKAGE_NEW,
				async (node: DataNode) => {
					newResource(node);
				},
			),
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_PACKAGE_NEW_JAVA_CLASS,
				async (node?: DataNode | Uri) => {
					newJavaFileWithSpecificType(JavaType.CLASS, node);
				},
			),
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_PACKAGE_NEW_JAVA_INTERFACE,
				async (node?: DataNode | Uri) => {
					newJavaFileWithSpecificType(JavaType.INTERFACE, node);
				},
			),
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_PACKAGE_NEW_JAVA_ENUM,
				async (node?: DataNode | Uri) => {
					newJavaFileWithSpecificType(JavaType.ENUM, node);
				},
			),
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_PACKAGE_NEW_JAVA_RECORD,
				async (node?: DataNode | Uri) => {
					newJavaFileWithSpecificType(JavaType.RECORD, node);
				},
			),
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_PACKAGE_NEW_JAVA_ANNOTATION,
				async (node?: DataNode | Uri) => {
					newJavaFileWithSpecificType(JavaType.ANNOTATION, node);
				},
			),
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_PACKAGE_NEW_JAVA_ABSTRACT_CLASS,
				async (node?: DataNode | Uri) => {
					newJavaFileWithSpecificType(JavaType.ABSTRACT_CLASS, node);
				},
			),
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_PACKAGE_NEW_FILE,
				async (node: DataNode) => {
					newFile(node);
				},
			),
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_PACKAGE_NEW_FOLDER,
				async (node: DataNode) => {
					newFolder(node);
				},
			),
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_PACKAGE_NEW_JAVA_PACKAGE,
				async (node?: DataNode) => {
					let cmdNode = getCmdNode(
			...
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_EXPLORER_NEW_PACKAGE,
				(node: Uri) => {
					newPackage(node);
				},
			),
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_PACKAGE_REVEAL_FILE_OS,
				(node?: DataNode) => {
					const cmdNode = getCmdNode(
						this._dependencyViewer.selection,
						node,
					);
					...
					if (cmdNode?.uri) {
						commands.executeCommand(
							"revealFileInOS",
							Uri.parse(cmdNode.uri),
						);
					}
				},
			),
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_PACKAGE_COPY_FILE_PATH,
				(node?: DataNode) => {
					const cmdNode = getCmdNode(
						this._dependencyViewer.selection,
						node,
			...
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_PACKAGE_COPY_RELATIVE_FILE_PATH,
				(node?: DataNode) => {
					const cmdNode = getCmdNode(
						this._dependencyViewer.selection,
						node,
			...
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_PACKAGE_RENAME_FILE,
				(node?: DataNode) => {
					renameFile(
						getCmdNode(this._dependencyViewer.selection, node),
					);
				},
			),
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_PACKAGE_MOVE_FILE_TO_TRASH,
				(node?: DataNode) => {
					deleteFiles(
						getCmdNode(this._dependencyViewer.selection, node),
						true,
			...
			instrumentOperationAsVsCodeCommand(
				Commands.VIEW_PACKAGE_DELETE_FILE_PERMANENTLY,
				(node?: DataNode) => {
					deleteFiles(
						getCmdNode(this._dependencyViewer.selection, node),
						false,
	...
	public async reveal(
		uri: Uri,
		needCheckSyncSetting: boolean = true,
	): Promise<void> {
		try {
