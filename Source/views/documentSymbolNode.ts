...
export class DocumentSymbolNode extends ExplorerNode {
	...
	constructor(
		private readonly symbolInfo: DocumentSymbol,
		parent: PrimaryTypeNode,
	) {
		super(parent);
	}
	...
	public getChildren(): ExplorerNode[] | Promise<ExplorerNode[]> {
		...
		if (this.symbolInfo?.children?.length) {
			this.symbolInfo.children.forEach((child) => {
				res.push(
					new DocumentSymbolNode(
						child,
						this.getParent() as PrimaryTypeNode,
					),
				);
			});
		}
	...
	public getTreeItem(): TreeItem | Promise<TreeItem> {
		const item = new TreeItem(
			this.getDisplayName(),
			this.symbolInfo?.children?.length
