		...
		try {
			switch (nodeData.kind) {
				...
				case NodeKind.Container:
					if (!parent || !project) {
						throw new Error(
							"Container node must have parent and project.",
						);
					}
				...
				case NodeKind.PackageRoot:
					if (!parent || !project) {
						throw new Error(
							"Package root node must have parent and project.",
						);
					}

					if (isHierarchicalView) {
						return new HierarchicalPackageRootNode(
							nodeData,
							parent,
							project,
						);
				...
				case NodeKind.Package:
					if (!parent || !project || !rootNode) {
						throw new Error(
							"Package node must have parent, project and root.",
						);
					}

					if (nodeData instanceof HierarchicalPackageNodeData) {
						return new HierarchicalPackageNode(
							nodeData,
							parent,
							project,
							rootNode,
				...
				case NodeKind.PrimaryType:
					if (
						nodeData.metaData &&
						nodeData.metaData[PrimaryTypeNode.K_TYPE_KIND]
					) {
						if (!parent) {
							throw new Error(
								"Primary type node must have parent.",
							);
						}
				...
				case NodeKind.Folder:
					if (!parent || !project) {
						throw new Error(
							"Folder node must have parent and project.",
						);
					}
