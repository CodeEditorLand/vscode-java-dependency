...
import {
	IPackageRootNodeData,
	PackageRootKind,
} from "../java/packageRootNodeData";
...
export class PackageRootNode extends DataNode {
	...
	public isSourceRoot(): boolean {
		return (
			(<IPackageRootNodeData>this.nodeData).entryKind ===
			PackageRootKind.K_SOURCE
		);
	}
	...
	protected createChildNodeList(): ExplorerNode[] {
		...
		if (this.nodeData.children && this.nodeData.children.length) {
			this.nodeData.children.forEach((nodeData) => {
				result.push(
					NodeFactory.createNode(nodeData, this, this._project, this),
				);
			});
		}
	...
	protected get contextValue(): string {
		...
		if (data.entryKind === PackageRootKind.K_BINARY) {
			...
			if (
				parent.path?.startsWith("REFERENCED_LIBRARIES_PATH") &&
