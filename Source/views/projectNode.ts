	...
	public async revealPaths(
		...
		const childNode = <DataNode>children.find((child: DataNode) => {
			if (child instanceof HierarchicalPackageNode) {
				return (
					childNodeData.name.startsWith(child.nodeData.name + ".") ||
	...
	protected createChildNodeList(): ExplorerNode[] {
		...
		if (packageData.length > 0) {
			if (Settings.isHierarchicalView()) {
				const data: HierarchicalPackageNodeData =
				...
				if (data) {
					result.push(
						...data.children.map((d) =>
							NodeFactory.createNode(d, this, this, this),
						),
					);
				}
			} else {
